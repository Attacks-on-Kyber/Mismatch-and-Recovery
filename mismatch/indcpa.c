...
static void pack_ciphertext_Attack(unsigned char *r, polyvec *b, poly *v)
{
  ...
  for(int i=0;i<KYBER_N;i+=8)//取v 8个16bit参数
  {
    for(int j=0;j<8;j++)
      t[j] = (uint32_t)v->coeffs[i+j];//8个16bit参数压缩到t的8个位置
    //这里取消了压缩，以防止对c2设置值的改变，设置的值在压缩的范围之内
    r[0] = t[0] | (t[1] << 4);
    r[1] = t[2] | (t[3] << 4);
    r[2] = t[4] | (t[5] << 4);
    r[3] = t[6] | (t[7] << 4);
         ...
         int h, int k, int select)
{
  ...
  k == 0 ? (Pb.vec[select].coeffs[0] = (int)(KYBER_Q/16.0 + 0.5)) 
         : (Pb.vec[select].coeffs[256-k] = (int)(-(KYBER_Q/16.0 + 0.5)));
...
/*********** Key Recovery Attack ****************/
void oracle_recovery(uint8_t r[256],
                const polyvec PA,
                const uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES])
{
